...
function ThoughtBlock({ message, block, onToggleThought }: { message: ChatMessageModel; block: ChatMessageModel["blocks"][number]; onToggleThought: (messageId: string, blockId: string) => void }) {
  ...
  return (
    <div className={`w-full max-w-full transition-all duration-300 ${block.isArchived ? "opacity-25" : "opacity-100"}`}>
      <button
        type="button"
        onClick={() => onToggleThought(message.id, block.id)}
        className="flex items-center gap-2 text-[10px] font-semibold tracking-wider text-muted-foreground/55 hover:text-foreground transition-colors duration-200 px-1 py-1 rounded hover:bg-muted/20"
      >
        {block.isCollapsed ? <ChevronRight size={11} strokeWidth={2} /> : <ChevronDown size={11} strokeWidth={2} />}
        <div className="relative flex items-center justify-center">
          <span className={`h-[5px] w-[5px] rounded-full ${block.isStreaming ? "bg-primary shadow-[0_0_5px_rgba(38,92,178,0.4)]" : "bg-primary/40"}`} />
          {block.isStreaming && <span className="absolute h-[5px] w-[5px] rounded-full bg-primary/30 animate-ping" />}
        </div>
        <span className="uppercase">{block.isStreaming ? "Thinking" : "Thought"}</span>
      </button>
      <div
        ref={scrollRef}
        className={`mt-0.5 ml-2.5 pl-3.5 pr-3 py-2 rounded-r-md text-[11px] text-muted-foreground/60 leading-relaxed transition-all duration-300 overflow-x-hidden overflow-y-auto ${block.isCollapsed ? "max-h-0 opacity-0 py-0 mt-0" : "max-h-36 opacity-100"}`}
        style={{
          background: "linear-gradient(135deg, rgba(38,92,178,0.035) 0%, rgba(38,92,178,0.01) 100%)",
          borderLeft: "2px solid rgba(38,92,178,0.2)",
          borderTop: "1px solid rgba(38,92,178,0.05)",
          borderRight: "1px solid rgba(38,92,178,0.03)",
          borderBottom: "1px solid rgba(38,92,178,0.03)",
        }}
      >
  ...
  if (status === "running") {
    return (
      <span className="relative flex items-center justify-center w-3 h-3">
        <span className="absolute w-3 h-3 rounded-full bg-primary/15 animate-ping" />
        <span className="w-1.5 h-1.5 rounded-full bg-primary shadow-[0_0_3px_rgba(38,92,178,0.3)]" />
      </span>
    );
  }
  if (status === "done") {
...
function ActionBlock({ block, onToggleThought, message }: { block: ChatMessageModel["blocks"][number]; onToggleThought: (messageId: string, blockId: string) => void; message: ChatMessageModel }) {
  ...
  return (
    <div className={`w-full max-w-full transition-all duration-300 ${isArchived ? "opacity-30" : "opacity-100"}`}>
      <button
        type="button"
        onClick={() => onToggleThought(message.id, block.id)}
        className="flex items-center gap-2 text-[10px] font-semibold tracking-wider text-muted-foreground/55 hover:text-foreground transition-colors duration-200 px-1 py-1 rounded hover:bg-muted/20"
      >
        ...
        {isStreaming ? (
          <span className="relative flex items-center justify-center">
            <span className="h-[5px] w-[5px] rounded-full bg-emerald-500 shadow-[0_0_5px_rgba(16,185,129,0.4)]" />
            <span className="absolute h-[5px] w-[5px] rounded-full bg-emerald-500/30 animate-ping" />
          </span>
        ) : (
      ...
      <div
        className={`mt-0.5 ml-2.5 pl-3.5 pr-3 py-2 rounded-r-md text-[11px] leading-relaxed transition-all duration-300 overflow-hidden ${isCollapsed ? "max-h-0 opacity-0 py-0 mt-0" : "max-h-52 opacity-100 overflow-y-auto"}`}
        style={{
          background: "linear-gradient(135deg, rgba(16,185,129,0.035) 0%, rgba(16,185,129,0.01) 100%)",
          borderLeft: "2px solid rgba(16,185,129,0.2)",
          borderTop: "1px solid rgba(16,185,129,0.05)",
          borderRight: "1px solid rgba(16,185,129,0.03)",
          borderBottom: "1px solid rgba(16,185,129,0.03)",
        }}
      >
    ...
    <div className={`flex w-full mb-5 ${isUser ? "justify-end" : "justify-start"}`}>
      <div className={`flex gap-2.5 max-w-[88%] ${isUser ? "flex-row-reverse" : "flex-row"}`}>
        <div className="flex-shrink-0 mt-0.5">
          <div className={`w-7 h-7 rounded-lg flex items-center justify-center ${isUser ? "bg-primary/8 text-primary" : isSystem ? "bg-amber-50 text-amber-600" : "bg-gradient-to-br from-primary/10 to-primary/3 text-primary border border-primary/8"}`}>
            {isUser ? <User size={14} strokeWidth={1.8} /> : isSystem ? <Terminal size={14} strokeWidth={1.8} /> : <img src="/floodmind-icon.svg" alt="FloodMind" className="w-4 h-4" />}
          </div>
        </div>

        <div className={`flex flex-col gap-1.5 ${isUser ? "items-end" : "items-start"}`}>
          {isUser ? (
            <div className="px-3.5 py-2 rounded-xl rounded-tr-sm bg-primary text-primary-foreground shadow-[0_1px_6px_-2px_rgba(38,92,178,0.12)] text-[13px] whitespace-pre-wrap leading-relaxed">
              {message.content}
            </div>
          ) : isSystem ? (
            <div className="px-3.5 py-2.5 rounded-xl rounded-tl-sm bg-amber-50/70 border border-amber-200/50 text-[13px] text-amber-900 leading-relaxed whitespace-pre-wrap font-mono">
              {message.content}
            </div>
          ) : (
            <div className="flex flex-col gap-2.5 w-full">
              {message.blocks.filter((block) => !block.isArchived).map((block) => {
                ...
                return (
                  <div key={block.id} className={`px-3.5 py-2.5 rounded-xl rounded-tl-sm bg-card border border-border/45 text-[13px] text-foreground leading-relaxed transition-all duration-300 shadow-[0_1px_3px_-1px_rgba(0,0,0,0.02)] ${block.isArchived ? "opacity-40 scale-[0.995]" : "opacity-100"}`}>
                        ...
                        {message.artifacts.map((artifact) =>
                          artifact.type === "image_generated" ? (
                            <div
                              ...
                              className="w-[35%] min-w-[200px] overflow-hidden rounded-lg border border-border/60 bg-card text-sm hover:border-primary/25 transition-all duration-200 group"
                            >
                              {artifact.image_url && (
                                <div
                            ...
                            <div
                              ...
                              className="w-[35%] min-w-[200px] rounded-lg border border-border/60 bg-card text-sm hover:border-primary/25 transition-all duration-200 overflow-hidden"
                            >
