...
本 skill 用于执行敖江案例水文接口 `/aj/hydro_model`，并且必须在这里直接完成站点识别、任务拆解、`input.json`构建。
...
```bash
python run_aojiang_hydro_model.py --input_file "input.json" [--output_file "result.json"] [--excel_output_file "result.xlsx"] [--base_url "http://192.168.30.108:3500"]
```
...
```
Bash(
    command='python run_aojiang_hydro_model.py --input_file input.json --output_file result.json --base_url http://192.168.30.108:3500',
...
| `牛溪流域出口` | 支流出口计算任务 | 否 | 1 | `GE2AF000000R` |
| `桂湖溪流域出口` | 支流出口计算任务 | 否 | 1 | `GE2AG000000L` |
