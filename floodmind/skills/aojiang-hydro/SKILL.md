...
## 使用原则
- 本 skill 只负责调用敖江案例水文接口 `/aj/hydro_model_sync`
- 执行时只允许通过 `--input_file` 传入完整 `input.json`
- 任务执行路径： 任务类型判断-->站点与`stationCode` 映射-->处理用户传入数据-->构建标准`input.json`文件-->完整核对`input.json`文件中的数据与用户上传数据是否一致（如不一致则回退到构建标准`input.json`文件步骤）-->执行接口-->完整检查返回数据的中的降雨数据是否与用户上传数据一致（不一致就回退到处理用户数据步骤）
...
- 如`计算山仔水库的入库流量` ，根据站点标准名与别称的内容可知霍口水库在山仔水库上游，那就必须考虑霍口水库上游降雨和霍口到山仔水库的区间降雨了
- 如`计算流域出口流量过程`，根据站点标准名与别称的内容可知该任务对应`temp-2`站点流量过程计算，那么`霍口水库` `山仔水库``temp-1``牛溪流域出口``桂湖溪流域出口` 都在其上游，都需要考虑
...
| `桂湖溪流域出口断面预报` | `GE2AF000000R` | 桂湖溪支流出口任务 |
| `牛溪流域出口断面预报` | `GE2AG000000L` | 牛溪支流出口任务 |
...
- stationCode：`33c76b8bd9384486a945c2fc7fd622eb`、`20001`、`30001`、`40001`、`GE2AF000000R`、`GE2AG000000L`
...
| `牛溪流域出口` | 支流出口计算任务 | 否 | `GE2AG000000L` |
| `桂湖溪流域出口` | 支流出口计算任务 | 否 | `GE2AF000000R` |
...
## input.json文件构建要求（更详细的内容请查阅 补充说明 部分）
...
```bash
python run_aojiang_hydro_model.py --input_file "input.json" [--output_file "result.json"] [--excel_output_file "result.xlsx"] [--base_url "http://192.168.30.108:11111"]
```

参数：
- `--input_file`：完整请求体 JSON 文件路径
- `--output_file`：保存结构化结果 JSON；默认 `result.json`
- `--excel_output_file`：自定义结果 Excel 保存路径；默认与 `--output_file` 同名 `.xlsx`
- `--base_url`：可选，服务基础地址
...
```
Bash(
    command='python run_aojiang_hydro_model.py --input_file input.json --output_file result.json --base_url http://192.168.30.108:11111',
    description='Run Aojiang hydro model'
)
```

上面这段调用完成后，会同时生成 `result.json` 和 `result.xlsx`。

如果服务未启动，需要先启动 `all_flask.py`，否则脚本会返回网络连接错误。


## 补充说明(更详细的input.json构建示例)

1. 识别用户提到的站点或别称。
2. 判断任务类型。
3. 判断是否需要补充上游站点。
4. 将任务拆解为一个或多个子预报任务。
5. 为每个子任务找到正确的 `stationCode`。
6. 按拆解结果准备 `input.json`。
...
#### 2. 只有“联合预报任务”才需要考虑主干流上游依赖
- 当目标站点位于敖江主干流，且其结果依赖上游主干流过程时，属于联合预报任务。
- 联合预报任务必须补充该目标位置所依赖的全部上游子任务。

#### 3. 支流出口任务不考虑主干流上游站点
- `牛溪流域出口` 和 `桂湖溪流域出口` 是支流出口任务。
- 计算这两个站点时，只使用各自对应的 `stationCode`。
- 不要补充霍口水库、山仔水库、`temp-1`、`temp-2` 等主干流站点。

#### 4. 区间任务默认不补充上游站点
- 例如“霍口水库到山仔水库区间流量”“山仔水库到 temp-1 区间流量”。
- 这类任务直接使用该区间对应的 `stationCode`。
- 不要再向上递归补充更上游站点。
...
#### 6. `temp-1` 和 `temp-2` 是虚拟站点
- 它们用于任务拆解和主干流拓扑表达。
- 用户通常不会直接点名这两个站点，但 agent 在联合预报拆解时可以使用它们。
...
这里的位置关系仅表示各站点在敖江流域主干河流上的相对上下游顺序。
...
### 四、预报子任务与 stationCode 映射

| 子任务名称 | stationCode | 使用场景 |
|---|---|---|
| `霍口水库断面预报` | `33c76b8bd9384486a945c2fc7fd622eb` | 霍口水库单断面预报；或作为联合预报上游组成部分 |
| `霍口水库~山仔水库区间断面预报` | `20001` | 山仔水库联合预报组成部分；或该区间单独预报 |
| `山仔水库~temp-1 区间断面预报` | `30001` | `temp-1` 或更下游联合预报组成部分；用户通常称为“山仔水库~水动力模型区间断面预报” |
| `temp-1~temp-2 区间断面预报` | `40001` | 主干流最终出口联合预报组成部分；用户通常称为“水动力模型区间断面预报” |
| `桂湖溪流域出口断面预报` | `GE2AF000000R` | 桂湖溪支流出口任务 |
| `牛溪流域出口断面预报` | `GE2AG000000L` | 牛溪支流出口任务 |

### 五、任务类型判定规则

收到用户请求后，先把任务归类为以下四类之一。

#### A. 联合预报任务
适用情形：

- 目标是主干流某个下游位置的最终流量或入库流量。
- 该目标结果依赖多个上游主干流子过程共同组成。
...
- `霍口水库到山仔水库区间未来3小时流量`
- `山仔水库到temp-1区间未来3小时流量`

处理原则：

- 直接使用对应区间 `stationCode`。
- 不再补充更上游站点。
...
#### 步骤 4：拆解为子预报任务
- 将目标任务转换成一个或多个标准子任务名称。

#### 步骤 5：映射到 `stationCode`
- 每个子任务都必须准确映射到对应 `stationCode`。

#### 步骤 6：准备 `input.json`
- `modelDataParams` 中填写历史降雨输入。
- `modelForecastRainfallParams` 中填写未来降雨输入。
- 每条输入数据都必须带对应的 `stationCode`。

### 七、联合预报的标准拆解规则

这一节最重要，agent 在主干流任务上必须优先参考这里。

#### 示例 1：用户要求计算山仔水库未来 3 小时入库流量
...
```json
{
  ...
  "modelDataParams": [
    ...
    {"time": "2023-06-30 00:00:00", "stationCode": "GE2AF000000R", "rainfallValue": 7.2},
    {"time": "2023-07-01 00:00:00", "stationCode": "GE2AF000000R", "rainfallValue": 7.8},
    {"time": "2023-06-30 00:00:00", "stationCode": "GE2AG000000L", "rainfallValue": 1.2},
    {"time": "2023-07-01 00:00:00", "stationCode": "GE2AG000000L", "rainfallValue": 1.8}
  ],
  "modelForecastRainfallParams": [
    {"time": "2023-07-01 01:00:00", "stationCode": "33c76b8bd9384486a945c2fc7fd622eb", "rainfallValue": 10.5},
    {"time": "2023-07-01 02:00:00", "stationCode": "33c76b8bd9384486a945c2fc7fd622eb", "rainfallValue": 11.2},
    ...
    {"time": "2023-07-01 01:00:00", "stationCode": "GE2AF000000R", "rainfallValue": 3.5},
    {"time": "2023-07-01 02:00:00", "stationCode": "GE2AF000000R", "rainfallValue": 3.2},
    {"time": "2023-07-01 03:00:00", "stationCode": "GE2AF000000R", "rainfallValue": 3.3},
    {"time": "2023-07-01 01:00:00", "stationCode": "GE2AG000000L", "rainfallValue": 4.5},
    {"time": "2023-07-01 02:00:00", "stationCode": "GE2AG000000L", "rainfallValue": 4.2},
    {"time": "2023-07-01 03:00:00", "stationCode": "GE2AG000000L", "rainfallValue": 4.3}
  ],
  "modelRunParam": {
    "XAJ": []
  }
...
#### 示例 3：用户要求计算牛溪流域出口未来 3 小时最终出口流量
...
#### 示例 4：用户要求计算霍口水库到山仔水库区间未来 3 小时流量
...
- 当用户说“敖江流域最终出口流量”时，按主干流最终出口 `temp-2` 理解。
- 此时需要同时纳入主干流过程和两条支流出口过程。

### 八、快速决策表

| 用户目标 | 任务类型 | 是否补充上游 | 子任务数量 | stationCode |
|---|---|---|---|---|
| `霍口水库` | 单断面预报任务 | 否 | 1 | `33c76b8bd9384486a945c2fc7fd622eb` |
| `山仔水库` | 联合预报任务 | 是 | 2 | `33c76b8bd9384486a945c2fc7fd622eb`、`20001` |
| `牛溪流域出口` | 支流出口计算任务 | 否 | 1 | `GE2AG000000L` |
| `桂湖溪流域出口` | 支流出口计算任务 | 否 | 1 | `GE2AF000000R` |
| `霍口水库~山仔水库区间` | 区间预报任务 | 否 | 1 | `20001` |
| `山仔水库~temp-1区间` | 区间预报任务 | 否 | 1 | `30001` |
| `temp-1~temp-2区间` | 区间预报任务 | 否 | 1 | `40001` |
| `敖江流域最终出口` | 联合预报任务 | 是 | 6 | `33c76b8bd9384486a945c2fc7fd622eb`、`20001`、`30001`、`40001`、`GE2AG000000L`、`GE2AF000000R` |

### 九、agent 执行时的口径要求

agent 在内部推理或输出任务拆解时，应尽量使用以下固定口径：

- `目标站点/区间是什么？`
- `这属于哪一种任务类型？`
- `是否需要补充上游站点？`
- `要拆成哪些标准子任务？`
- `每个子任务对应哪个 stationCode？`

## result.json 示例文件

以下是一个真实的 `result.json` 输出示例（敖江案例 `case_name` 为 `"敖江"`），展示完整的请求与响应结构：
...
```json
{
  ...
  "success": true,
  "request": {
    "base_url": "http://192.168.30.108:11111",
    "endpoint": "/aj/hydro_model_sync",
    "payload": {
      "forecastTime": "2024-07-26 08:00:00",
      "historyDuration": "2",
      "futureDuration": "2",
      "modelDataParams": [
        {"time": "2024-07-25 08:00:00", "stationCode": "33c76b8bd9384486a945c2fc7fd622eb", "rainfallValue": 1.506265},
        {"time": "2024-07-25 09:00:00", "stationCode": "33c76b8bd9384486a945c2fc7fd622eb", "rainfallValue": 4.009406}
      ],
      "modelForecastRainfallParams": [
        {"time": "2024-07-26 08:00:00", "stationCode": "33c76b8bd9384486a945c2fc7fd622eb", "rainfallValue": 3.593557},
        {"time": "2024-07-26 09:00:00", "stationCode": "33c76b8bd9384486a945c2fc7fd622eb", "rainfallValue": 2.506961}
      ],
      "modelRunParam": {
        "XAJ": []
      }
  ...
  "response": {
    "code": 200,
    "data": [
      {"flow": "345.11", "modelType": "XAJ", "sectionCode": "33c76b8bd9384486a945c2fc7fd622eb", "time": "2024-07-25 08:00:00", "rainfallValue": 1.506265},
      {"flow": "355.08", "modelType": "XAJ", "sectionCode": "33c76b8bd9384486a945c2fc7fd622eb", "time": "2024-07-25 09:00:00", "rainfallValue": 4.009406}
    ]
  }
}
```