...
class SkillCurator:
    ...
    def __init__(
        ...
        # 默认：项目级 .floodmind/skills/ + 全局 ~/.floodmind/skills/
        if skills_dirs is None:
            skills_dirs = [str(Path.cwd() / ".floodmind" / "skills"), str(Path.home() / ".floodmind" / "skills")]
    ...
    def archive_skill(self, skill_name: str) -> bool:
        ...
        archive_dir = base_dir / ".archive" / skill_name
        try:
    ...
    def _find_archive_dir(self, skill_name: str) -> Optional[Path]:
        """在所有 skills_dirs 中查找归档目录"""
        for d in self.skills_dirs:
            archive_dir = d / ".archive" / skill_name
            if archive_dir.exists():
                return archive_dir
        return None

    def restore_skill(self, skill_name: str) -> bool:
        """从归档恢复 skill（恢复到原所在目录）"""
        archive_dir = self._find_archive_dir(skill_name)
        if archive_dir is None:
            logger.warning("Archived skill %s not found", skill_name)
            return False

        # 恢复到原目录（归档目录的父目录的父目录即原 skills_dir）
        skill_dir = archive_dir.parent.parent / skill_name
        try:
            if skill_dir.exists():
