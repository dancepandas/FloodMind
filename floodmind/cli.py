...
@main.group()
def session():
    """会话管理 (fork/revert/compact/export)"""
    pass


@session.command()
@click.argument("session_id")
@click.option("--up-to", "-u", default=None, help="分叉到指定 message_id（不含后续消息）")
def fork(session_id, up_to):
    """分叉会话：创建独立副本，保留到指定消息为止"""
    from floodmind.memory import fork_session, get_session
    s = get_session(session_id)
    if not s:
        click.echo(f"错误: 会话 {session_id} 不存在")
        return
    new_id = fork_session(session_id, up_to_message_id=up_to)
    click.echo(f"[OK] 已分叉: {session_id} → {new_id}")


@session.command()
@click.argument("session_id")
@click.argument("message_id")
def revert(session_id, message_id):
    """回退会话：删除指定消息之后的所有消息"""
    from floodmind.memory import revert_session
    revert_session(session_id, message_id)
    click.echo(f"[OK] 已回退会话 {session_id} 到消息 {message_id}")


@session.command()
@click.argument("session_id")
def compact(session_id):
    """压缩会话：用 LLM 摘要替代早期消息以节省上下文"""
    from floodmind.memory import compact_session
    summary = compact_session(session_id)
    if summary:
        click.echo(f"[OK] 已压缩会话 {session_id}: {summary[:120]}...")
    else:
        click.echo(f"会话 {session_id} 消息不足，无需压缩")
...
def session_list():
    """列出所有会话"""
    from floodmind.memory import list_sessions
    ...
    for s in sessions:
        ...
        sid = s.get("id", "")
        click.echo(f"  {sid[:12]}  {title[:30]:30s}  messages: {msg_count:3d}  {updated}")
