...
class CapabilityRegistry:
    ...
    def resolve_capabilities(self, provider: str, family: str, model: str) -> Tuple[ModelCapabilities, CapabilitySource]:
        ...
        # family override 与 exact override 各自只覆盖显式设置的字段，逐层继承（§7.6）。
        base = self._provider_defaults.get(provider) or ModelCapabilities()
        merged, source = base, CapabilitySource.provider_default
        if (provider, family) in self._family:
            merged = ModelCapabilities(**{
                **merged.model_dump(),
                **self._family[(provider, family)].model_dump(exclude_unset=True),
            })
            source = CapabilitySource.family
        if model in self._exact:
            merged = ModelCapabilities(**{
                **merged.model_dump(),
                **self._exact[model].model_dump(exclude_unset=True),
            })
            source = CapabilitySource.exact
        return merged.model_copy(deep=True), source
