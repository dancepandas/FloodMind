...
> **更新**: 2026-08-06 — SDK v1.1.6；`SearchTools` 输出改为目录风格（仅工具名 + 基本描述，参数统一走 `GetTool`），`short_description` 剥离 `[必填]/[可选]` 参数提示前缀。v1.1.5 含四项健壮性/权限收敛：① 工具调用参数键名统一清洗（模型偶发畸形键如 `{"tool_name"": ...}` 不再 `**kwargs` 崩）；② exec 命令体写目标检查（`>`/`Set-Content`/`Copy-Item` 等越权写 DENY，堵住"只读授权被 Bash 绕过"）；③ folder-first 读白名单加入已装 skill 注册表；④ PathService 读取拒绝原因附可操作引导。v1.1.4 含 create() 连接阶段 LLM 流式重试
...
```bash
pytest tests/ -q          # v1.1.6 core-only: 600 passed, 1 skipped
...
├── scheduler.py                      # 定时任务调度
├── tests/                            # 测试（v1.1.6 core-only: 600 passed, 1 skipped）
├── docs/                             # 文档
