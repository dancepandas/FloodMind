...
> **更新日期**: 2026-08-05
> **变更摘要**: SDK v1.1.0；公共 `Agent` 新增 `bare=False` 完整 runtime 入口、`memory`/`session_id`/`clear_memory` 代理、`stream(**kwargs)` 转发；MCP 层新增 tool-name sanitize（OpenAI 兼容端点）、stdio liveness、call health、server-connected listener；`_build_model_info` 读取宿主已路由的 `ModelClient.model_name`。
...
Checkpoint 在 v1.1.0 中只保存 Agent runtime state：`state.json` 是 `AgentLoopState` 序列化，`manifest.json` 记录 checkpoint 元数据和父 checkpoint 链。它不复制 workspace 文件，也不承担文件回滚职责；产物通过 artifact/journal 记录引用，文件版本化或回滚如需支持应由独立 change journal / artifact versioning 能力承担。
