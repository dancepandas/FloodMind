...
> **更新日期**: 2026-07-04
> **变更**: 标记所有已完成批次，新增本次 Skill/MCP 重构后评估。架构地图见 [`OVERVIEW.md`](./OVERVIEW.md)。
...
| P0-8 | `web_server.py:813-824` | web `create_agent_for_session` 硬编码记忆参数，忽略 `settings.agent` | ⏳ |
...
| 位置 | 问题 |
|---|---|
| `native_flood_agent.py:804` | `update_plan` 工具 `is_readonly=False` 但 `policy_type="readonly"` — 语义矛盾 |
