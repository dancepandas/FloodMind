...
def test_exec_policy_asks_for_mutating_redirection(tmp_path):
    ws = build_folder_workspace("s1", primary_dir=tmp_path / "project").ensure()
    decision = PermissionService(
        path_service=PathService(project_root=tmp_path, workspace=ws)
    ).check_tool_policy(
...
def test_impl_exec_bash_requires_cwd_without_context(tmp_path):
    set_session_context("", output_dir="")
    set_runtime_context(RuntimeContext("", "", "", "", "", path_service=PathService(project_root=tmp_path, workspace=None)))
