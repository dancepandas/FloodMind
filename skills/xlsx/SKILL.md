...
description: >
  ...
  database pipeline, or Google Sheets API integration, even if tabular data is
  involved.
...
```
run_script(skill_name='xlsx', script_name='脚本名.py', args='[...]')
```
...
如果需要先生成临时脚本、JSON 或 CSV 文件，优先使用 `write_text_file` 写文件，再调用 `exec_python_file` 执行。

`exec_bash` 仅用于真正的 PowerShell 命令，不要再用 `exec_bash(command='python ...')` 承载 Python 脚本执行。

## 创建文件

### create_excel.py - 从 JSON 数据创建 Excel 文件

用于把结构化 JSON 数据直接导出为新的 Excel 文件，适合预测结果、统计表、列表型数据导出。

**适用场景：**
- 需要新建 `.xlsx` 文件
- 已经有结构化结果数据（如对象数组或二维数组）
- 不涉及复杂公式建模

**用法：**
```bash
python create_excel.py --output "path/to/output.xlsx" --rows "[...]" [options]
```
...
|------|------|--------|------|
| --output | ✅ | - | 输出文件绝对路径 |
| --rows | ✅ | - | JSON 数组，支持对象数组或二维数组 |
| --columns | ❌ | - | 列名 JSON 数组；当 `rows` 为二维数组时必填 |
...
run_script(
    skill_name='xlsx',
    script_name='create_excel.py',
    args='["--output", "D:\\data\\result.xlsx", "--rows", "[{\"时间\": \"2025-03-25 12:30:00\", \"预测流量(m³/s)\": 164.02}]"]'
)
```
...
预览 CSV/Excel/TXT/JSON 文件的前几行，用于快速了解数据结构。

**必要条件：**
- 文件必须存在
- 文件大小不超过 50MB
- 支持的格式：CSV (.csv)、Excel (.xlsx, .xls, .xlsm)、TXT (.txt)、JSON (.json)
