#!/usr/bin/env python3
"""把敖江水文模型结构化 JSON 结果导出为 Excel 文件。"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="把敖江水文模型结构化 JSON 结果导出为 Excel 文件")
    parser.add_argument("--input_file", required=True, help="run_aojiang_hydro_model.py 生成的结构化 JSON 文件")
...
def main() -> None:
    ...
    detail_count, failed_count = export_to_excel(input_path, output_path)
    print(f"Excel 文件已创建: {output_path}")
    print(f"结果明细行数: {detail_count}")
    print(f"生成工作表数量: {failed_count}")
