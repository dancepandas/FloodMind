...
```bash
python run_aojiang_hydro_model.py --input_file "input.json" [--output_file "result.json"] [--base_url "http://192.168.30.108:3500"]
```

参数：
- `--input_file`：完整请求体 JSON 文件路径
- `--output_file`：可选，保存结构化结果 JSON
- `--base_url`：可选，服务基础地址
- `--timeout`：可选，请求超时时间，默认 `120`

### export_aojiang_hydro_result_to_excel.py

把 `run_aojiang_hydro_model.py --output_file` 生成的结构化 JSON 按断面拆分为 Excel。

```bash
python export_aojiang_hydro_result_to_excel.py --input_file "result.json" [--output_file "result.xlsx"]
```

规则：
- 一个断面生成一个 sheet
- sheet 名使用 `stationCode`
- 每个 sheet 保存该断面的全部字段和全部记录
...
run_script(
    ...
    args=['--input_file', 'outputs/aojiang_input.json', '--output_file', 'outputs/aojiang_result.json', '--base_url', 'http://192.168.30.108:3500']
)
```
...
run_script(
    skill_name='aojiang-hydro-intake',
    script_name='export_aojiang_hydro_result_to_excel.py',
    args=['--input_file', 'outputs/aojiang_result.json', '--output_file', 'outputs/aojiang_result.xlsx']
)
```
