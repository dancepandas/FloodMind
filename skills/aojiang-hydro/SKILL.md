...
- 任务执行路径： 任务类型判断-->站点与`stationCode` 映射-->处理用户传入数据-->构建标准`input.json`文件-->完整核对`input.json`文件中的数据与用户上传数据是否一致（如不一致则回退到构建标准`input.json`文件步骤）-->执行接口-->完整检查返回数据的中的降雨数据是否与用户上传数据一致（不一致就回退到处理用户数据）
