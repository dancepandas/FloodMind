...
- `--output_file`：可选，保存结构化结果 JSON；默认输出到当前目录下 `result.json`
- `--excel_output_file`：可选，自定义结果 Excel 保存路径；默认与 `--output_file` 同名 `.xlsx`
- `--base_url`：可选，服务基础地址
...
上面这段调用完成后，会同时生成 `outputs/aojiang_result.json` 和 `outputs/aojiang_result.xlsx`。
